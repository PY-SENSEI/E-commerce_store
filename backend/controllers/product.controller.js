...
export const getAllProducts = async (req, res) => {
    try {
        const products = await Product.find({}); //it will get everything
        res.json({ products });
    } catch (error) {
...
export const getFeaturedProducts = async (req, res) => {
    try {
       ...
       //if not in redis, fetch it from db
...
export const getRecommendedProducts = async (req, res) => {
    try {
        ...
        console.log("Error in the getRecommendedProducts controller", error.message);
        res.status(500).json({ message: error.message });
    }
}
